...
def test_jwt_exp_not_int():
    payload = {"sub": "1", "type": "access", "jti": "123", "exp": "not_int"}
...
def test_decode_expired_token():
    token = create_token({"sub": "1", "type": "access"}, timedelta(seconds=-5))
...
def test_decode_wrong_secret():
    token = jwt.encode(
        {"sub": "1", "type": "access"}, "wrong", algorithm=settings.ALGORITHM
    )
